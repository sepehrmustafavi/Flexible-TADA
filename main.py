...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        config["model"].get("model_name_or_path"),
    ...
    if config.get("system", {}).get("save_final_model", False):
        trainer.save_model()
    else:
        logger.info("Skipping final model save to save disk space.")

    logger.info(f"Training completed. Global Step: {train_result.global_step}")

    # Take Training Metrics 
